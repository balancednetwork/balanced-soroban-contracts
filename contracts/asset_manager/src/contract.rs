...
impl AssetManager {
    ...
    pub fn configure_rate_limit(
        ...
        write_token_current_limit(&env, &token_address, (contract_token_balance as u128) * percentage/POINTS);
        Ok(())
    }
    ...
    fn send_deposit_message(
       ...
       Self::transfer_token_to(&e, from.clone(), token.clone(), current_address.clone(), amount);
       ...
       let xcall_message: Deposit = Deposit::new(
        ...
        let (sources, destinations) = Self::xcall_manager(&e, &config.xcall_manager ).get_protocols();
        ...
        Self::xcall_client(&e, &config.xcall).send_call(&from, &current_address, envelope, &config.icon_asset_manager);
        Ok(())
    }
