...
impl TestContext {
    ...
    pub fn init_context(&self, client: &AssetManagerClient<'static>) {
        ...
        let config = ConfigData {xcall: self.xcall.clone(), xcall_manager: self.xcall_manager.clone(), native_address: self.native_token.clone(), icon_asset_manager: self.icon_asset_manager.clone()};
    ...
    pub fn init_xcall_state(&self){
        ...
        self.xcall_client.set_default_connection(&self.nid, &self.centralized_connection);
    }
