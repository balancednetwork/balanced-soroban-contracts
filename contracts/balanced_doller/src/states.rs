...
pub fn write_u64_state(e: &Env, key: DataKey, id: &u64) {
    e.storage().instance().set(&key, id);
}

pub fn write_u128_state(e: &Env, key: DataKey, id: &u128) {
    e.storage().instance().set(&key, id);
}
...
pub fn read_u64_state(e: &Env, key: DataKey) -> u64 {
    e.storage().instance().get(&key).unwrap()
}

pub fn read_i128_state(e: &Env, key: DataKey) -> i128 {
    e.storage().instance().get(&key).unwrap()
}

pub fn read_u128_state(e: &Env, key: DataKey) -> u128 {
    e.storage().instance().get(&key).unwrap()
}
