...
use crate::contract::{
    XcallManager, XcallManagerClient
};
...
impl TestContext {
    ...
    pub fn init_context(&self, client: &XcallManagerClient<'static>) {
        ...
        let config = ConfigData {xcall: self.xcall.clone(), icon_governance: self.icon_governance.clone()};
        ...
        client.initialize(&self.registry, &self.admin, &config, &sources, &destinations);
